...
function ProductGridSection({ s, primary, subdomain }: { s: MergedSection; primary: string; subdomain: string }) {
  const c = s.content as Record<string, unknown>;
...
function SectionSwitch({ s, primary, subdomain }: { s: MergedSection; primary: string; subdomain: string }) {
  if (!s.enabled) return null;
  switch (s.type) {
    case "hero":
      return <HeroSection s={s} primary={primary} />;
    case "product_grid":
      return <ProductGridSection s={s} primary={primary} subdomain={subdomain} />;
    case "image_gallery":
    ...
    >
      ...
      <main>
        {[...site.sections]
          ...
          .map((s) => (
            <SectionSwitch key={s.id} s={s} primary={primary} subdomain={site.subdomain} />
          ))}
      </main>
